...
const View = () => {
    ...
    const fetchEmployee = async () => {
      try {
        ...
        if (response.data.success) {
          setEmployee(response.data.employee); // Set employee data
        } else {
  ...
  if (!employee || !employee.userId) {
    return <div className="text-center text-red-500">Employee data is not available.</div>;
  }
    ...
    <div className="max-w-3xl mx-auto mt-10 bg-white p-8 rounded-md shadow-md">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          ...
          <img
            src={`http://localhost:5000/uploads/${employee.userId.profileImage || "default.jpg"}`}
